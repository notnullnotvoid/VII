...
{
	HMODULE res = LoadLibraryA("SDL2.dll"); //TODO: how the hell doesn't this fail!?
    return res;
}
...
static void * openDll()
{
	...
	res = dlopen("link/SDL2.framework/SDL2", RTLD_LAZY);
	if (!res) res = dlopen("link/SDL2.dylib", RTLD_LAZY);
